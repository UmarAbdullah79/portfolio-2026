...
const services = [
  {
    ...
    description:
      "Responsive, accessible, and scalable UI built with React & Next.js.",
    icon: <Layout className="w-6 h-6 text-emerald-400" />,
  },
  {
    title: "Full-Stack Foundations",
    description:
      "Robust APIs and secure authentication, prioritizing structure and logic.",
    icon: <Server className="w-6 h-6 text-emerald-400" />,
  },
  {
    title: "Performance & SEO",
    description:
      "Optimizing Core Web Vitals for fast loads and better visibility.",
    icon: <Zap className="w-6 h-6 text-emerald-400" />,
  },
  {
    title: "UI to Code Translation",
    description:
      "Converting Figma designs into pixel-perfect, interactive code.",
      ...
      <div className="max-w-8xl mx-auto w-full">
        ...
        <div ref={titleRef} className="max-w-3xl mb-16 md:mb-24">
          ...
          <p className="text-silver/60 text-lg md:text-xl font-light leading-relaxed max-w-xl">
            Blending technical performance with clear, usable design to solve
            real problems.
          </p>
        </div>
        ...
        >
          {services.map((service, index) => (
            <div
              key={index}
              className="group relative p-8 rounded-2xl bg-white/3 border border-white/5 hover:border-white/10 transition-all duration-500 hover:-translate-y-2"
            >
              {/* Soft hover glow effect */}
              <div className="absolute inset-0 rounded-2xl bg-linear-to-b from-white/8 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
